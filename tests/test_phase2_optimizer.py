...
class Phase2OptimizerTests(unittest.TestCase):
    ...
    def test_optimizer_records_correctness_failures(self) -> None:
        ...
        try:
            ...
            self.assertEqual(state_json["last_completed_iteration"], 1)
            ...
            self.assertEqual(report_json["last_completed_iteration"], 1)
    ...
    def test_optimizer_persists_partial_state_when_evaluator_raises(self) -> None:
        ...
        try:
            ...
            self.assertEqual(state_json["iteration"], 1)
            self.assertEqual(state_json["last_completed_iteration"], 0)
            self.assertFalse(state_json["done"])
            self.assertEqual(report_json["iterations_run"], 1)
            self.assertEqual(report_json["last_completed_iteration"], 0)
    ...
    def test_optimizer_extends_iteration_limit_after_correctness_passes(self) -> None:
        ...
        try:
            ...
            with patch.dict("os.environ", {"PROFILER_AGENT_PHASE2_SPEEDUP_ITERATIONS": "4"}, clear=False):
                result = run_phase2_optimization(
                    root_dir=root,
                    max_iterations=2,
                    candidate_generator=generator,
            ...
            self.assertEqual(result.iterations_run, 4)
            self.assertEqual(result.state.stop_reason, "max_iterations_reached:4")
            self.assertEqual(generated_ids, ["cand-1", "cand-2", "cand-3", "cand-4"])
        finally:
            shutil.rmtree(root, ignore_errors=True)
