...
export PROFILER_AGENT_PHASE2_ITERATIONS="${PROFILER_AGENT_PHASE2_ITERATIONS:-0}"
...
if [[ "${TIME_BUDGET_ENABLED}" == "1" || "${TIME_BUDGET_ENABLED}" == "true" || "${TIME_BUDGET_ENABLED}" == "yes" || "${TIME_BUDGET_ENABLED}" == "on" ]]; then
  ...
  echo "[run.sh] Phase2 iterations: ${PHASE2_ITERATIONS} (0 means time-budget driven), stop buffer: ${PROFILER_AGENT_PHASE2_STOP_BUFFER_SECONDS}s"
